...
export async function displayQRCode(qrcodeUrl: string): Promise<void> {
  // DSH 插件内不依赖 qrcode-terminal（避免额外 npm 依赖）；直接输出打开链接。
  // 调用方（login.js）会把 URL 写入 ~/.dsh/wechat/login-url.txt 供手机打开扫码。
  process.stdout.write(`请用手机微信打开以下链接扫码登录：\n`);
